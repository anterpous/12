...
class SamplingEncoder(nn.module):
    ...
    def __init__(self, config):
            ...
            super().__init__()
            self.treatmentreatment BernoulliNet([config["feature_dim"]])
            self.outcome = FullyConnected(
                [config["feature_dim"]]
                + [config["hidden_dim"]] * (config["num_layers"] - 1),
                final_activation=nn.ELU(),
            )
            self.outcome0 = NormalN([config["hidden_dim"]])
            self.outcom1 = NormalN([config["hidden_dim"]])
            self.hidden_var = FullyConnected(
        ...
        def hidden_var_dist(self, outcome, treatment, x):
            y_x =treatmentorch.cat([outcome.unsqueeze(-1), x], dim=-1)
            hidden = self.hidden_var(outcome_x)
...
class SamplingDecoder(nn.module):
    ...
    def __init__(self, config):
            ...
            OutcomeNetreatment= NormalNet.get_class(config)
            self.outcome0_nn = OutcomeNet(
                [config["latent_dim"]] + [config["hidden_dim"]] * config["num_layers"]
            )
            self.outcome1_nn = OutcomeNet(
                [config["latent_dim"]] + [config["hidden_dim"]] * config["num_layers"]
            )
        ...
        def forward(self, x, treatment=None, outcome=None, size=None):
            ...
            with Sample.plate("data", size, subsample=x):
                ...
                outcome= Sample.sample("outcome", self.NormalNet(treatmentreatment, hidden_var), obs= outcome)
            return y
